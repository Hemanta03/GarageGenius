...
/**
 * Staff internal messaging controller (staff ↔ admin).
 *
 * <p>Supports inbox/sent views, composing messages to admins, viewing messages, marking as read,
 * and viewing job-specific threads limited to the current user.</p>
 *
 * <p>Mapped to {@code /staff/messages}.</p>
 */
public class StaffMessageController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (action) {
            case "sent":
                ...
                request.getRequestDispatcher("/views/staff/messages/inbox.jsp").forward(request, response);
                break;
            case "view":
            ...
            case "compose":
                ...
                break;
            case "thread":
                showThread(request, response, userId);
                break;
            case "inbox":
            default:
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if ("send".equals(action)) {
            ...
            response.sendRedirect(request.getContextPath() + "/staff/messages?success=sent");
        } else if ("markRead".equals(action)) {
            int id = Integer.parseInt(request.getParameter("id"));
            messageDAO.markRead(id, userId);
            response.sendRedirect(request.getContextPath() + "/staff/messages");
        } else {
            response.sendRedirect(request.getContextPath() + "/staff/messages");
        }
    }
    ...
    private void viewMessage(HttpServletRequest request, HttpServletResponse response, int userId) throws IOException, ServletException {
        try {
            ...
            if (m.getJobId() != null) {
                request.setAttribute("thread", messageDAO.getJobThreadForUser(m.getJobId(), userId));
            }
            request.setAttribute("pageTitle", "View Message");
            request.getRequestDispatcher("/views/staff/messages/view.jsp").forward(request, response);
    ...
    private void showCompose(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("/views/staff/messages/compose.jsp").forward(request, response);
    }

    private void showThread(HttpServletRequest request, HttpServletResponse response, int userId) throws ServletException, IOException {
        String jobIdStr = request.getParameter("jobId");
        if (ValidationUtil.isEmpty(jobIdStr)) {
            response.sendRedirect(request.getContextPath() + "/staff/messages");
            return;
        ...
        request.setAttribute("thread", messageDAO.getJobThreadForUser(jobId, userId));
        request.setAttribute("jobId", jobId);
        request.setAttribute("pageTitle", "Job Messages");
        request.getRequestDispatcher("/views/staff/messages/thread.jsp").forward(request, response);
    }
}
